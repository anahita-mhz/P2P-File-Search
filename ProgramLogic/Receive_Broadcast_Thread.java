...
public class Receive_Broadcast_Thread extends Thread
{
	Config config;
	...
	public Receive_Broadcast_Thread(Config config,
			BlockingQueue<Network_Control_Message> delayed_queue,
			Broadcast_Causal_Info causal_info,
			Data_Collector data_collector,
			Reporter reporter)
	...
	public void run()
	{
		...
		while(true)
		{
			try {
				...
				Broadcast_Message msg = (Broadcast_Message)pckt.get_msg();
	...
	private void check_buffereds()
	{
		...
		while(counter > 0)
		{
			...
			else
			{
				...
				data_collector.add_time_buffered((int)(System.currentTimeMillis() - buff_pckt.buff_timestamp));
	...
	{
		Broadcast_Message msg = (Broadcast_Message) pckt.pckt.get_msg();
		...
		out += String.format("Message vector is %s\r\n", int_array_to_string(msg.msg_vector));
		out += String.format("Process vector is %s\r\n", int_array_to_string(causal_info.getVector()));
		System.out.print(out);
		reporter.screen(out);
	}
	...
	{
		Broadcast_Message msg = (Broadcast_Message) pckt.get_msg();
		causal_info.update(msg.msg_vector);
		int sender_node_id = msg.sender_id;
		String out = "Message from node " + sender_node_id + " with physical timestamp "
	...
	private String int_array_to_string(int[] array)
	{
		String str = "";
